...
namespace QLTV
{
    public partial class PhieuMuonSach : Form
    {
        ...
        private void bt_muon_Click(object sender, EventArgs e)
        {
            ...
            //    connection.Close();
            //}
            DateTime ngayMuon = dateM.Value;
            DateTime dateTra = date_hen_tra.Value;
            using (var connection = new SqlConnection(str))
            {
                connection.Open();

                // Tạo và cấu hình SqlCommand
                using (SqlCommand command = new SqlCommand("AddPhieuMuon", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("@MaPhieuMuon", maPM.Text);
                    command.Parameters.AddWithValue("@MaSach", maSachMuon.Text);
                    command.Parameters.AddWithValue("@SoLuongMuon", slMuon.Text);
                    command.Parameters.AddWithValue("@NgayMuon", ngayMuon.ToString("yyyy-MM-dd HH:mm:ss"));
                    command.Parameters.AddWithValue("@HenTra", dateTra.ToString("yyyy-MM-dd HH:mm:ss"));
                    command.Parameters.AddWithValue("@MaQuanLy", DATAOFSAKER.data_maQL);
                    command.Parameters.AddWithValue("@TinhTrang", tinhTrang.Text);
                    command.Parameters.AddWithValue("@MaDocGia", maDG.Text);
                    ...
                    {
                        MessageBox.Show("Số lượng sách không đủ trong kho", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        return;
                    }
                    else if (resultCode == 2)
                    {
                        MessageBox.Show("Mã sách mượn không tồn tại", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        return;
                    }
                    else if(resultCode == 3)
                    {
                        MessageBox.Show("Mã phiếu mượn đã tồn tại", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                        return;
                    }
                    LoadData();
                }
        ...
        private void bt_sua_Click(object sender, EventArgs e)
        {
            ...
            if (string.Compare(tinhTrang.Text, "Chưa trả") == 0)
            {
                DateTime ngayMuon = dateM.Value;
                DateTime dateTra = date_hen_tra.Value;
                using (var connection = new SqlConnection(str))
                {
                    ...
                    // Tạo và cấu hình SqlCommand
                    using (SqlCommand command = new SqlCommand("UpdatePhieuMuon", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@MaPhieuMuon", maPM.Text);
                        command.Parameters.AddWithValue("@MaSach", maSachMuon.Text);
                       ...
                       if(resultCode==0)
                        ...
                        {
                            MessageBox.Show("Mã phiếu mượn không tồn tại", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                            return;
                        }
            ...
            {
                MessageBox.Show("Trường hợp này đã xử lí rồi hoặc đã rơi vào quá hạn hãy kiểm tra lại","Thông báo",MessageBoxButtons.OK,MessageBoxIcon.Stop);
                return;
            }
        ...
        private void bt_xoa_Click(object sender, EventArgs e)
        {
            ...
            //    }
            //}
            if (string.Compare(tinhTrang.Text, "Đã trả") == 0)
            {
                using (var connection = new SqlConnection(str))
                {
                    connection.Open();
                    ...
                    // Tạo và cấu hình SqlCommand
                    using (SqlCommand command = new SqlCommand("DeletePhieuMuon", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@MaPM", maPM.Text);
                        command.ExecuteNonQuery();
            ...
            {
                MessageBox.Show("Sách chưa xử lí nên chưa thể xóa","Thông báo",MessageBoxButtons.OK,MessageBoxIcon.Stop);
                LoadData();
