        ...
        private void BAOCAO_PM_1_Load(object sender, EventArgs e)
        {
            ...
            if (DATA_RP.check == 1)
            {
                ...
                using (SqlConnection connection = new SqlConnection(str))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("ThuTucTimKiemTinhTrang", connection))
                    {
            ...
            else if (DATA_RP.check == 2)
            {
                using (SqlConnection connection = new SqlConnection(str))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("ThuTucTimKiemTinhTrangDaTra", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
            ...
            else if (DATA_RP.check == 3)
            {
                using (SqlConnection connection = new SqlConnection(str))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("TimKiemNgayMuonVaHenTraNamTrongThang", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
